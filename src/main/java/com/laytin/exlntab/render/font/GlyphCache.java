...
public class GlyphCache {
    ...
    private BufferedImage glyphCacheImage = new BufferedImage(2048, 2048, 2);
    ...
    public GlyphCache() {
        ...
        this.imageData = new int[4194304];
        this.imageBuffer = ByteBuffer.allocateDirect(16777216).order(ByteOrder.BIG_ENDIAN).asIntBuffer();
    ...
    void cacheGlyphs(Font font, char[] text, int start, int limit, int layoutFlags) {
        ...
        for(int index = 0; index < numGlyphs; ++index) {
            ...
            if (!this.glyphCache.containsKey(fontKey | (long)glyphCode)) {
                ...
                if (this.cachePosX + rect.width + 1 > 2048) {
                    this.cachePosX = 1;
                    this.cachePosY += this.cacheLineHeight + 1;
                    this.cacheLineHeight = 0;
                }

                if (this.cachePosY + rect.height + 1 > 2048) {
                ...
                entry.u1 = (float)rect.x / 2048.0F;
                entry.v1 = (float)rect.y / 2048.0F;
                entry.u2 = (float)(rect.x + rect.width) / 2048.0F;
                entry.v2 = (float)(rect.y + rect.height) / 2048.0F;
    ...
    private void allocateGlyphCacheTexture() {
        this.glyphCacheGraphics.clearRect(0, 0, 2048, 2048);
        this.singleIntBuffer.clear();
        GL11.glGenTextures(this.singleIntBuffer);
        this.textureName = this.singleIntBuffer.get(0);
        this.updateImageBuffer(0, 0, 2048, 2048);
        GL11.glBindTexture(3553, this.textureName);
        GL11.glTexImage2D(3553, 0, 6408, 2048, 2048, 0, 6408, 5121, this.imageBuffer);
